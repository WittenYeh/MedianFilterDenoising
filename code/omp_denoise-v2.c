...
unsigned char * median_filter(unsigned char *image, int width, int height, int filter_size) {
    ...
    unsigned char r_vals[filter_size * filter_size];
    unsigned char g_vals[filter_size * filter_size];
    unsigned char b_vals[filter_size * filter_size];

    for (int y = 0; y < height; y++) {
        #pragma omp parallel for private(r_vals, g_vals, b_vals)
